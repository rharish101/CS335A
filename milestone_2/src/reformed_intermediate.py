...
def symbol_table(tree, table, name=None, block_type=None, store_var="temp"):
            ...
            if len(lhs) != len(rhs) and len(rhs) != 0:
                ...
                for i, expr in enumerate(rhs):
                    expr_dtype, expr_code = symbol_table(
                        expr, table,store_var="__decl{}".format(i)
                    )
                    ir_code += expr_code
    ...
    # XXX still need to incorporate typedef alias during type checking
        ...
        for item in const_list:
            ...
            if len(id_list) != len(expr_list):
                ...
                for i, expr in enumerate(expr_list):
                    expr_dtype, expr_code = symbol_table(
                        expr, table,store_var="__const{}".format(i)
                    )
                    ir_code += expr_code
                    evaluated_types.append(expr_dtype)
                ...
                if isinstance(curr, GoPrimaryExpr):
                    if isinstance(curr.rhs, GoSelector):
                        ...
                        dtype, index_code = symbol_table(
                            curr.rhs.index, table,store_var="__index" + str(ind_cnt)
                        )
        ...
        dtype1, lhs_code = symbol_table(lhs, table, store_var="__lhs")
        dtype2, rhs_code = symbol_table(rhs, table, store_var="__rhs")
        ir_code += "{} = __lhs {} __rhs\n".format(store_var, op)
    ...
    #===================================================================
        ...
        print("tree.dtype {}, tree.value {}".format(tree.dtype.name,tree.value))
        ...
        if isinstance(tree.dtype, GoArray):
            ...
            # table.insert_var(struct_name, struct_obj, "struct")
            #XXX     
            DTYPE = struct_obj
    ...
    #==========================================================================
        ...
        if tree.op == "&" or tree.op == "*":
            ...
            elif isinstance(tree.expr, GoPrimaryExpr) or isinstance(tree.expr,GoFromModule):
                # print("XXX2")
